...
function searchLast(rx,text,extremeOffset){												//for the findRegexPrev(), searches the last occurence in a line
	var rx=new RegExp(rx)
	var index=-1,length,extremeReached=false

	var found=rx.exec(text)
	if(extremeOffset!==false){															//if there is alimitation by the current document selection
		if(found && found.index>=extremeOffset) extremeReached=true						//then search last term but before this limitation
		while(found && !extremeReached){
			index=found.index
